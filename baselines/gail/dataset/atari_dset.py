...
class Atari_Cset(object):
    frame_stack = 4

    def __init__(self, expert_path, cs_type, train_fraction=0.7, traj_limitation=-1, randomize=True):
        # with open(str(Path(expert_path)/'demo_trajs.pkl'),'rb') as f:
            # traj_data = pickle.load(f)
        traj_data = torch.load(expert_path+'.D')
        if cs_type!='full':
            cs_ids = torch.load(expert_path+'.Z')

        if traj_limitation < 0:
            traj_limitation = len(traj_data['S'])

        if cs_type=='full':
            obs = traj_data['S'][:traj_limitation]
            acs = traj_data['A'][:traj_limitation]
        else:
            obs = traj_data['S'][cs_ids[cs_type]][:traj_limitation]
            acs = traj_data['A'][cs_ids[cs_type]][:traj_limitation]

        self.obs = obs
        self.acs = acs.reshape(acs.shape[0],)
        # self.acs = np.concatenate(acs,axis=0)
        ...
        assert len(self.obs) == len(self.acs)
        self.num_traj = min(traj_limitation, len(traj_data['S']))
        self.num_transition = len(self.obs)
        self.randomize = randomize
        self.dset = Dset(self.obs, self.acs, self.randomize)
        # for behavior cloning
